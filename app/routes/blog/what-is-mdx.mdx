...
import BlogComponent from "~/components/table-logic";
...
<motion.div
  ...
  viewport={{ once: true }}
  transition={{
    y: { type: "spring", duration: 0.8, delay: 0.4 },
    opacity: { delay: 0.4 },
  }}
>
...
<motion.div
  ...
  viewport={{ once: true }}
  transition={{
    x: { type: "spring", duration: 0.8, delay: 0.3 },
    opacity: { delay: 0.3 },
  }}
>
<div className="transition ease-out hover:scale-105">
...
>
<div className="transition ease-out hover:scale-105 rounded-lg overflow-hidden bg-[#1c2128]">
![Routing to mdx](https://i.imgur.com/gFnvqxj.png)
</div>
</motion.div>
</motion.div>
...
<BlogComponent heading="This Component" description="Components can be utilized as we normally do, and it will display in the .mdx file." />

</motion.div>

Here is what the inside of that component looks like... 👀, yes I know, I'm a great magician 🪄 ...
...
#### Not only can you reference other components within the MDX, we can pull data from the MDX.
...
<div className="flex">
  <Link to={`../code-highlighting`} className="transition ease-in-out
    ...
    <div>
      <h4 style={{ marginTop: "0px" }} className="pt-4 pb-2 px-5 text-white">✍️ Code Highlighting 👉</h4>
    </div>
  </Link>
</div>
</motion.div>
