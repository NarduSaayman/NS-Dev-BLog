...
I had an interesting problem to solve. 🤔 A user would query some name with a starting and ending date 📅 and get back a list of results of some generic data and a status associated to each item. Here is where the problem comes in.
...
I would then find a odd use case for `new Set( )`, where I would take taht array with all the duplicates and set itself to a an array of a new set of itself... yes, I know, I don't know what I was thinking there...
...
Great, I now have the record, now I'll need to just add it to the new array to udpate their statuses. Easy enough, just push onto the array... but I'm doing a lot of changes and using `array.push()` can lead to some unwanted changes to the data in the array. A better approach is to then spread the original and then appending the new content.
...
Nice, now I'm starting to work and manipualte the array a bit better, but one thing is till a problem though is what if there is a duplicate?

Rather than pushing duplicates onto the array, check if there is a unique entry in the array and then update that entry. I'll just check if that `find()` I did earlier found a value, if it does great, I have something to index on. If it doesn't have anything I can jsut add the new value. 
